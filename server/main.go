...
func orderby(sourceresult string, n string, h string) (order string) {
	FuncStorage = map[string]interface{}{
		"drank":     drank,
		"gamecount": gamecount,
		"viewdecks": viewdecks,
	}
	...
	// Set key/value pairs using typical `name[key] = val`
	m["k6"] = fmt.Sprintf("%-30s", "Return to Menu")
	m["k7"] = fmt.Sprintf("%0s", "Quit")

	switch sourceresult {
	case "drank":
		m["k1"] = fmt.Sprintf("%-30s", "Rank")
		m["k2"] = fmt.Sprintf("%0s", "Deck Name")
		m["k3"] = fmt.Sprintf("%-30s", "Wins")
		m["k4"] = fmt.Sprintf("%0s", "Loses")

		fmt.Println("Change Order of Results")
		fmt.Println("1:", m["k1"], " 2:", m["k2"])
		fmt.Println("3:", m["k3"], " 4:", m["k4"])
	case "gamecount":
		m["k1"] = fmt.Sprintf("%-30s", "Deck Name")
		m["k2"] = fmt.Sprintf("%0s", "Game Count")

		fmt.Println("Change Order of Results")
		fmt.Println("1:", m["k1"], " 2:", m["k2"])
	case "viewdecks":
		m["k1"] = fmt.Sprintf("%-30s", "Deck Name")
		m["k2"] = fmt.Sprintf("%0s", "Colors")
		m["k3"] = fmt.Sprintf("%-30s", "Date Entered")
		m["k4"] = fmt.Sprintf("%0s", "Favorite")
		m["k5"] = fmt.Sprintf("%-30s", "Max Streak")
	...
	fmt.Println("Test " + order)
	Call(sourceresult, n, h, order)
	return
}
...
func drank(DeckName string, h string, order string) error {
	// Open up our database connection.
	db := opendb()
	// defer the close till after the main function has finished
	// executing
	...
	switch order {
	case "Rank":
		rkquery_all = rkquery_all + " ORDER BY ranking"
	case "Deck Name":
		rkquery_all = rkquery_all + " ORDER BY deck"
	case "Wins":
		rkquery_all = rkquery_all + " ORDER BY wins"
	case "Loses":
		rkquery_all = rkquery_all + " ORDER BY loses"
	}
	...
	fmt.Println("")
	orderby("drank", DeckName, h)
	//menu()
	return nil
}
func gamecount(d string, h string, order string) {
	db := opendb()
	// executing
	defer db.Close()
	...
	case "all":
		...
		if order != "Game Count" {
			ctquery = "SELECT deck, results AS Count FROM mtga.game_count WHERE deleted IS NULL ORDER BY deck"
		}
...
func viewdecks(DeckName string, edit int, h string, order string) (ret string) {
	// Open up our database connection.
	db := opendb()
	defer db.Close()
	...
	if h == "y" {
		...
		vquery_all = "SELECT name, colors, date_entered, favorite, max_streak FROM mtga.decks_all ORDER BY " + order_col
	} else {
		...
		vquery_all = "SELECT name, colors, date_entered, favorite, max_streak FROM mtga.decks ORDER BY " + order_col
	}
	if DeckName != "n" {
		var d Deck
		DeckName = strings.TrimSuffix(strings.TrimSuffix(DeckName, "\r"), "\n")
...
func Call(funcName string, params ...interface{}) (result interface{}, err error) {
	...
	if len(params) != f.Type().NumIn() {
		err = errors.New("number if params is wrong")
		return
	}
